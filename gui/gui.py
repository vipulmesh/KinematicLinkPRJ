...
class MainWindow(QMainWindow):
    ...
    def _on_start(self) -> None:
        ...
        # ── Stage 3: Grashof check (strict validation) ────────────────────────
        is_grashof, mtype = self.animation_area.solver.grashof()
        if not is_grashof:
            self.statusBar().showMessage("🔴 Invalid Mechanism")
            QMessageBox.critical(
                self,
                "Invalid Mechanism / Non-Grashof",
                "The entered link lengths produce a Non-Grashof mechanism.\n\n"
                "A Non-Grashof mechanism cannot make a full revolution and is not supported by this simulation.\n\n"
                "Please modify the link lengths."
            )
            return
    ...
    def _on_kinematics_update(self, data: dict) -> None:
        ...
        import math
        theta2_rad = math.radians(data['theta2'])
        theta3_rad = math.radians(data['theta3'])
        ...
        v_ax = -omega2 * l2 * math.sin(theta2_rad)
        v_ay = omega2 * l2 * math.cos(theta2_rad)
        v_a = math.sqrt(v_ax**2 + v_ay**2)

        v_bx = v_ax - omega3 * l3 * math.sin(theta3_rad)
        v_by = v_ay + omega3 * l3 * math.cos(theta3_rad)
        v_b = math.sqrt(v_bx**2 + v_by**2)
        ...
        txt = (
            ...
            f"Linear Velocity\n\n"
            f"Joint A\n"
            f"Velocity : {v_a:.2f} mm/s\n\n"
            f"Joint B\n"
            f"Velocity : {v_b:.2f} mm/s\n\n"
